...
class SurveyThreeViewController: UIViewController {
    ...
    func navigateToNext(){ //navigate without navigation controll
        ...
        let mianStorybord = UIStoryboard(name:"Main", bundle: Bundle.main)
         guard let surveyTwoVC = mianStorybord.instantiateViewController(withIdentifier: "SurveyFourViewController") as?
             SurveyFourViewController else{
                 return
         }
    ...
    func postData(setSurveyStatus surveyStatus:Int){
      ...
      let values = [
        "surveyThreeAnswer":surveyStatus
            ]as [String : Any]
